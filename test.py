...
class RubiksCubeDetector:
    ...
    def detect_cube(self, frame):
        ...
        blur_value = max(1, blur_value * 2 + 1)  # Ensure blur value is odd
    ...
    def detect_colors(self, roi):
        ...
        h, w, _ = hsv.shape
        center_pixel = hsv[h//2, w//2]  # Get the center pixel HSV value
        
        detected_color = "unknown"
        for color, (lower, upper) in self.hsv_ranges.items():
            lower_bound = np.array(lower)
            upper_bound = np.array(upper)
            if np.all(lower_bound <= center_pixel) and np.all(center_pixel <= upper_bound):
